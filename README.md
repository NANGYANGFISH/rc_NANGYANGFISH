...
| NOTIFY_ALLOWED_ORIGINS | 必填 | 出站通知目标白名单（不是浏览器 CORS 配置）；逗号分隔的精确 origin（协议、主机及端口），例如 https://api.example.com,http://other.example.com:8081；不能带业务路径或查询参数 |
...
AI 参与了需求拆解、方案讨论、代码实现、测试补充和安全审查。最终方案由我作为提交者，结合题目约束、现有代码和 MVP 目标进行 review，并对是否采用 AI 建议负责。

### 我主动拒绝的 AI 建议及原因

- **重建 Spring Boot 骨架**：当前仓库已有轻量 Java/H2 实现，能够支撑通知服务的核心链路。仅为框架偏好换栈会增加迁移和验证成本，因此我选择复用现有实现。
- **在单进程 MVP 中引入租约抢占和分布式协调**：当前只有一个进程和一个投递线程，没有多实例竞争认领的需求。提前加入租约、分布式锁会增加状态和故障场景，留到多实例确有必要时再设计。
- **首版增加独立投递记录表和统计接口**：本版先用任务状态与最后一次结果支撑查询、排障和人工重投，不为追求功能齐全扩展数据模型与接口。完整尝试审计和统计能力应由真实运维需求驱动。

对于基础设施选型，我也明确选择首版不引入 MQ 或 Redis：数据库任务表已能完成持久化与到期调度，额外组件会增加双写一致性、部署和排障成本。不是组件越多就越可靠，而是要先说明每个故障窗口如何处理。

### 我采纳的建议与最终取舍

我采纳了提交幂等、有限重试与退避、重启恢复、目标白名单与 IP 校验、真实数据库及 HTTP 集成测试等建议，因为它们直接保护当前可靠投递链路，而不是提前建设通用平台。

我的核心取舍是先完成“持久化 → 投递 → 重试 → 恢复 → DEAD → 人工补偿”的闭环，接受单进程和单投递线程的吞吐与可用性限制，不承诺永久必达或恰好一次。后续根据吞吐、失败率、积压年龄和运维需求演进，而不是为了展示技术栈提前堆叠组件。测试覆盖及尚未验证的范围见下一节。
